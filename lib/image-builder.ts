...
import { PHPDockerImageSource } from './image-source';
import { PHPDockerImageDestination } from './image-destination';
...
export interface PHPDockerImageBuilderProps {
  /**
   * We'll accept a source type to make it easier for devs to understand what this lib supports
   */
  readonly source: PHPDockerImageSource;

  /**
   * Destination service were hosting the image. just ecr for now
   */
  readonly destination: PHPDockerImageDestination;

}

export class PHPDockerImageBuilder extends Construct {
  private readonly project: Project;


  constructor(scope: Construct, id: string, props: PHPDockerImageBuilderProps) {
    super(scope, id);
